...
{
    class AnimatedTexture : ICompositeTexture
    {
        internal static string A_TextureRegexEnd = @"_A[0-9]+\.tga";
        internal static string A_TextureRegexEndTex = @"_A[0-9]+-c\.tex";
        internal static string A_TextureRegex = @"_A[0-9]+";

        internal static string NullElement = "_A{0}.TGA";
        internal static string NullElementTex = "_A{0}-C.TEX";
        ...
        public string GetNameBase() => Name;

        public string GetCompositeStatus()
        {
            return String.Format("({0} animation frames)", VariationCount);
        }
        ...
        public AnimatedTexture(FileInfo textureFile)
        {
        ...
        public void Add(FileInfo textureFile)
        {
            ...
            {
                int index;
                if (TryGetVariation(textureFile.Name, out index))
                {
                ...
                {
                    throw new ArgumentException("Texture file could not be parsed and added into animated texture!!!");
                }
            }
            throw new ArgumentException("Texture file could not be added into animated texture, names mismatch!!!");
        }

        private bool TryGetVariation(string name, out int variation)
        {
            var match = Regex.Match(name, A_TextureRegex, RegexOptions.IgnoreCase);
            if (match.Success)
            {
